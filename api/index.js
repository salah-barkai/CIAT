...
/* JSON bundlé par esbuild sur Vercel, lu depuis le disque en local */
import knowledge from './ciat-knowledge.json' with { type: 'json' };

const __dirname = dirname(fileURLToPath(import.meta.url));

/* Chargement explicite du .env depuis le répertoire api/ */
...
if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY === 'METS_TA_CLE_ICI') {
  if (process.env.VERCEL) {
    ...
    console.error('\n❌ ANTHROPIC_API_KEY manquante dans api/.env');
    console.error('   → Va sur https://console.anthropic.com → API Keys → Create Key');
    console.error('   → Colle la clé dans api/.env\n');
    process.exit(1);
  }
}
...
const buildPrompt = async () => {
  ...
  /* ── Section BUREAU : MongoDB (bilingue, live) + JSON (détails) ── */
